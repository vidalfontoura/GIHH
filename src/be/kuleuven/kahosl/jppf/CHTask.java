...
/**
 * This class involves a task information including the hyper-heuristic and the
 * target problem
 */
public class CHTask extends JPPFTask implements Serializable {
	...
	/**
	 * Phase length denoting the number of iterations to check the heuristics'
	 * performance (for the selection mechanism)
	 **/
	 ...
	 *            heuristic selection mechanism type
	 * @param phaseLength
	 *            phase length as number of iterations
	 * @param acceptance
	 *            acceptance mechanism type
	 * @param problemName
	 *            name of the target problem
	 * @param instanceIndex
	 *            instance index denoting the problem instance to solve
	 * @param totalExecutionTime
	 *            total execution time allowed in milliseconds
	 * @param resultSubFolderName
	 *            subfolder name where the generated files are saved
	 * @param resultFileName
	 *            common part of the files' name including the hyper-heuristic
	 *            information
	 * @param seed
	 *            seed value for the random number generator
	 */
	public CHTask(SelectionMethodType selection, int phaseLength, AcceptanceCriterionType acceptance,
			ProblemName problemName, int instanceIndex, long totalExecutionTime, String resultSubFolderName,
			String resultFileName, long seed) {
	...
	public void run() {
		...
		System.out.println(
				selection + "_PL" + phaseLength + "_" + acceptance + "_" + problemName + "_INST#" + instanceIndex);
		...
		try {
			...
			hh = new GIHH(seed, problem.getNumberOfHeuristics(), Vars.totalExecutionTime, resultFileName, selection,
					acceptance);
