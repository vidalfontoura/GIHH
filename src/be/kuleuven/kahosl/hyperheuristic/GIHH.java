...
public class GIHH extends HyperHeuristic implements Serializable {
	...
	/**
	 * The common part of the files' name including the hyper-heuristic
	 * information
	 **/
	...
	/**
	 * The number of new best solutions by the relay hybridisation (including
	 * the solutions found/visited where the first heuristic does not make no
	 * change)
	 **/
	...
	/**
	 * An array of integers denoting the ruin-and-recreate heuristics' index
	 * values
	 **/
	...
	/**
	 * Whether the current phase check to measure the heuristics' performance is
	 * the first check
	 **/
	...
	/**
	 * An array of units showing the speed of the heuristics with respect to the
	 * fastest non-tabu heuristic
	 **/
	...
	/**
	 * The number of iterations spent by the relay hybridisation (both a first
	 * heuristic and a second heuristic are used)
	 **/
	...
	/**
	 * A ratio used to determine to choose single heuristics or relay
	 * hybridisation (heuristic pairs)
	 **/
	 ...
	 * @param totalExecTime
	 *            the total execution time given (stopping criterion)
	 * @param resultFileName
	 *            the common part of the files' name including the
	 *            hyper-heuristic's details
	 * @param selectionType
	 *            the heuristic selection mechanism type
	...
	private void initializeHH() {
		statFiles.statFileInitialisation(resultFileName); // also create new
															// files for the
															// next run
	...
	/**
	 * Set the heuristic types. These type information is used only while
	 * applying heuristics. For GIHH, each heuristic is initially classified as
	 * ImprovingMoreOrEqual. This feature makes GIHH generic for different
	 * heuristic sets
	 * 
	 * @param problem
	 *            a target problem
	...
	protected void solve(ProblemDomain problem) {
		...
		Vars.restartSearch = false; // Reset value for the next run (the
									// variable is static)
		...
		/*
		 * Copy the initial solution to the best solution location in the
		 * solution memory
		 */
		...
		/*
		 * Take a number of copies of the initial solution for using the
		 * crossover operators (or any operator requiring two solutions)
		 */
		...
		lastRestartTimePoint = startTime; /*
											 * Set the start time as the most
											 * recent time point where the
											 * solution re-initialised
											 */

		/* Write some initial information to the corresponding stat files */
		// statFiles.writeIntoFile(WriteFileType.BFFile,
		// getCurrentStateForBestAsStr(-1));
		// statFiles.writeIntoFile(WriteFileType.HCntFile,
		// getHeuristicCounterDetailsAsStr());
		// statFiles.writeIntoFile(WriteFileType.ITFile,
		...
		while (!hasTimeExpired()) {
			...
			if (!Vars.useOverallBestSln && numberOfRestarts > 0
					&& (System.currentTimeMillis() - startTime) > (1.1 * (totalExecTime / 2.0))
			...
			if (Vars.restartSearch) {
				...
				if (numberOfRestarts > 0) {
					if ((System.currentTimeMillis() - lastRestartTimePoint)
							+ shortestRestartTime > (1.1 * (totalExecTime / 2.0))) {
						Vars.restartSearch = false;
						Vars.useOverallBestSln = true;
					...
					} else if (numberOfRestartsWithoutNewBest >= 5
							&& (double) (System.currentTimeMillis() - startTime) > ((double) totalExecTime / 5)) { // @13062011
			...
			if (Vars.restartSearch) {
				...
				Vars.restartSearch = false; /*
											 * Do not re-initialise the solution
											 */
				...
				/*
				 * Set the current best solution (index=10 in the solution
				 * memory) as the overall best solution (Index=11)
				 */
				if (numberOfRestarts > 0) {
					if (bestFitness < problem.getFunctionValue(11)) {
						problem.copySolution(10, 11);
						bestSlnFoundAsStr = problem.solutionToString(11);
					...
					/*
					 * Keep the shortest execution time spent after
					 * re-initialisations
					 */
				...
				/*
				 * Take copies of the new initial solution to use for crossover
				 * (or any heuristic requiring two solutions)
				 */
				if (crossover_heuristics != null) {
					for (int mInx = 5; mInx < 10; mInx++) {
						problem.copySolution(0, mInx); // for crossovers
				...
				if (Print.hyperheuristic) {
					log.info("RE-INITIALIZATION !!");
			...
			} else if (Vars.useOverallBestSln && numberOfRestarts > 0) { // If
																			// the
																			// search
																			// was
																			// restarted
																			...
																			// disabled
				/*
				 * Start using the overall best solution found after all
				 * re-initialisations. Also, use the threshold list when the
				 * overall best solution found, in the acceptance mechanism
				 */
				...
				/*
				 * Update the overall best solution if the most recent solution
				 * is better than the current overall best solution
				 */
				if (bestFitness > problem.getFunctionValue(11)) {
					if (Print.hyperheuristic) {
					...
					/*
					 * Set fitness values to the fitness of the overall best
					 * solution
					 */
					bestFitness = currentFitness = newFitness = problem.getFunctionValue(0);

					/*
					 * Change the threshold list of the acceptance mechanism
					 * with the threshold list when the overall best solution
					 * found
					 */
					acceptance.useNoMoreRestartCaseList();

					/*
					 * Write overall best solution information after
					 * re-initalisation to the BF file
					 */
					// statFiles.writeIntoFile(WriteFileType.BFFile,
					// getCurrentStateForBestAsStr(-1));
				}
			...
			/*
			 * Calculate the selection ratio between single heuristics and the
			 * relay hybridisation
			 */
			fcSelectPow = (double) (totalNumOfNewBestFound - numberOfFCNewBest + 1.0) / (numberOfFCNewBest + 1.0);
			/*
			 * Keep the selection ratio between single heuristics and the relay
			 * hybridisation within its bounds (0.02, 50)
			 */
			if (fcSelectPow > 50) {
				fcSelectPow = 50;
			...
			double fcSelectProb = Math.pow((double) (phaseIterCounter % Vars.phaseLength) / Vars.phaseLength,
					fcSelectPow);
			if (fcMode && tempProb <= fcSelectProb) {
				 ...
				 */
				/*
				 * Keep the starting time information for the execution of the
				 * first heuristic
				 */
				relayStartFirstHeur = System.nanoTime();
				if (isCrossover(
						wrsHeur)) { /* If the selected heuristic is crossover */
				...
				/*
				 * Keep the ending time information for the execution of the
				 * first heuristic
				 */
				relayEndFirstHeur = System.nanoTime();

				/*
				 * Get the fitness value (quality) of the solution after the
				 * first heuristic applied by the relay hybridisation
				 */
				relayFirstFitness = problem.getFunctionValue(1);

				/*
				 * If the solution generated/visited after the first heuristic
				 * applied is the same solution before it was applied, decrease
				 * its probability to be selected
				 */
				...
				/*
				 * If the first heuristic applied by the relay hybridisation
				 * finds a new best solution, do not apply the second heuristic
				 * (cancel the relay hybridisation). Otherwise, if the solution
				 * is a worsening solution, then apply a second heuristic
				 */
				if (relayFirstFitness >= bestFitness) {
					 ...
					 */
					/*
					 * Keep the starting time information for the execution of
					 * the first heuristic
					 */
					...
					/*
					 * Get the fitness value (quality) of the solution after the
					 * second heuristic applied by the relay hybridisation
					 */
					...
					if (newFitness < bestFitness) {
						/*
						 * Update the selection probabilities of the first
						 * heuristics in the relay hybridisation
						 */
						if (!punisProbListForRelay) { /*
														 * if the generated
														 * solution by the first
														 * heuristic is not the
														 * same as the solution
														 * before the relay
														 * hybridisation
														 */
							newBestByRelay = true;
							selection.updateProbListForRelay(wrsHeur, relaySecondHeurIndex, 1,
									learningRateMultiplierList); // @12052011
						} else { /*
									 * Punish the first heuristic since it
									 * generated the exact same solution
									 */
							selection.updateProbListForRelay(wrsHeur, relaySecondHeurIndex, 0,
									learningRateMultiplierList); // @12052011
						}

						/*
						 * The number of new best solutions by the relay
						 * hybridisation
						 */
						numberOfFCNewBestNotActual++;

						/*
						 * Add the second heuristic to the first heuristic's
						 * second heuristic list for the relay hybridisation,
						...
						// statFiles.writeIntoFile(WriteFileType.RHFile,
						// getRelayDetailsAsStr(relayFirstHeurIndex,
						// relaySecondHeurIndex, relayFirstFitness,
						// relaySecondFitness));

						/*
						 * If the solution generated/visited after the first
						 * heuristic applied is the same solution before it was
						 * applied
						 */
						if (!punisProbListForRelay) {
							numberOfFCNewBest++;
							currPerformance.setFCNumberOfImprovingBestMoves(
									currPerformance.getFCNumberOfImprovingBestMoves() + 1);
						}
																 ...
																 */
						if (!punisProbListForRelay) {
							selection.updateProbListForRelay(wrsHeur, relaySecondHeurIndex, 2,
									learningRateMultiplierList);
						} else { /*
									 * Punish the first heuristic since it
									 * generated/visited the same solution
									 * before it applied
									 */
							selection.updateProbListForRelay(wrsHeur, relaySecondHeurIndex, 0,
									learningRateMultiplierList);
								 ...
								 * If the solution found by the relay
								 * hybridisation is a worsening solution
								 */
						selection.updateProbListForRelay(wrsHeur, relaySecondHeurIndex, 0, learningRateMultiplierList);
					}
				} else { /*
							 * The second heuristic is not applied. Consider
							 * only the first applied heuristic
							 */
			...
			if (!isRelay) {
				...
				selection.updateSelectionElements(lastCalledHeuristic, heuristicClassTypeList[lastCalledHeuristic],
						currentFitness, newFitness, bestFitness, startHeur, endHeur, learningRateMultiplierList,
						currPerformance);
			} else { /* Relay hybridisation */
				/** First heuristic **/
				currPerformance.updatePerformanceElements(relayFirstHeurIndex, currentFitness, relayFirstFitness,
				...
				selection.updateSelectionElements(relayFirstHeurIndex, heuristicClassTypeList[relayFirstHeurIndex],
						currentFitness, relayFirstFitness, bestFitness, relayStartFirstHeur, relayEndFirstHeur,
						learningRateMultiplierList, currPerformance);
						/*********************************************************************************************************************/

				/** Second heuristic **/
				currPerformance.updatePerformanceElements(relaySecondHeurIndex, relayFirstFitness, relaySecondFitness,
				...
				selection.updateSelectionElements(relaySecondHeurIndex, heuristicClassTypeList[relaySecondHeurIndex],
						relayFirstFitness, relaySecondFitness, bestFitness, relayStartSecondHeur, relayEndSecondHeur,
						learningRateMultiplierList, currPerformance);
				/*********************************************************************************************************************/
			}

			/*
			 * Ask the acceptance mechanism whether to accept or reject the new
			 * solution
			 */
			if (acceptance.accept(newFitness, currentFitness, bestFitness)) {
				/*
				 * Copy the accepted solution to the memory as the current
				 * solution
				 */
				...
				if (newFitness < bestFitness) {
					...
					/*
					 * Increment the total number of new best solutions found
					 * counter
					 */
					...
					/*
					 * Change randomly one of the solution used for crossovers
					 * (or any heuristic requiring two solutions)
					 */
					...
					/*
					 * Append the new search state information to the
					 * corresponding stat files
					 */
					// statFiles.writeIntoFile(WriteFileType.BFFile,
					// getCurrentStateForBestAsStr(lastCalledHeuristic));
					// statFiles.writeIntoFile(WriteFileType.HCntFile,
					// getHeuristicCounterDetailsAsStr());
					// statFiles.writeIntoFile(WriteFileType.ITFile,
					// getIterationLimitDetailsAsStr());
					// statFiles.writeIntoFile(WriteFileType.LOCFile,
					// getCurrentStateForLOCAsStr());
					// statFiles.writeIntoFile(WriteFileType.LRMFile,
					// getCurrentStateForLRMAsStr());
					// statFiles.writeIntoFile(WriteFileType.IMPRFile,
					// getCurrentStateForIMPRAsStr());
					//
					// statFiles.writeIntoFile(WriteFileType.RHFile,
					// getRelayDetailsAsStrJustInfo());
				...
				// statFiles.writeIntoFile(WriteFileType.ACFile,
			...
			/*
			 * End of the current phase for the selection mechanism. Perform the
			 * required changes
			 */
			if (selectionType == SelectionMethodType.AdaptiveLimitedLAassistedDHSMentorSTD
					&& phaseIterCounter == Vars.phaseLength) {
				/*
				 * Check the performance changes of the heuristics and update
			...
			/*
			 * Write the current status of the search to the corresponding stat
			 * files
			 */
		...
		writeTheBestSlnToAFile(problem, bestFitness, problem.solutionToString(11));
	 ...
	 * 
	 * @param problem
	 *            target problem used to solve
	 * @param bestFitness
	 *            the best fitness value found
	 * @param bestSlnFoundAsStr
	 *            the best solution in a string
	 */
	public void writeTheBestSlnToAFile(ProblemDomain problem, double bestFitness, String bestSlnFoundAsStr) {
		...
		try {
			outBestSln = new BufferedWriter(new FileWriter(
					WriteInfo.mainFolder + WriteInfo.resultSubFolderName + "/" + resultFileName + "_BESTslnFOUND.txt",
					true));
	...
	private void performanceCheckForDHS() {
		...
		if (Vars.timeBasedPhaseLength) {
			...
			if (avgIterTime == 0) {
				Vars.phaseLength = Vars
						.calculatePhaseLength(numberOfHeuristics - selection.getNumberOfTabuHeuristics());
			} else {
				int tempPhaseLength = (int) (Vars.phaseDuration / (avgIterTime * 1000));

				if (tempPhaseLength < (numberOfHeuristics - selection.getNumberOfTabuHeuristics())
						* Vars.minPhaseFactor) {
					Vars.phaseLength = (numberOfHeuristics - selection.getNumberOfTabuHeuristics())
							* Vars.minPhaseFactor;
	...
	/**
	 * Update the units showing the speed of the heuristics with respect to the
	 * fastest non-tabu heuristic
	 * 
	...
	private void updateLearningRateMultiplier(int selectedHeuristicIndex) {
		...
		currPerformance.getNumberOfMovesPerTime()[selectedHeuristicIndex] = currPerformance
				.getNumberOfMoves()[selectedHeuristicIndex] / tempExecTime;
		...
		if (currPerformance.getNumberOfMovesPerTime()[selectedHeuristicIndex] == 0.0) {
			...
			Vars.learningRateMultiplier = (int) (baseRatioForLRate
					/ currPerformance.getNumberOfMovesPerTime()[selectedHeuristicIndex]);
		}
	...
	/**
	 * Calculate the performance of the heuristics at the end of a phase based
	...
	private void updatePerformanceMetricForSelection() {
		...
		for (int i = 0; i < numberOfHeuristics; i++) {
			if (currPerformance.getNumberOfImprovingBestMoves()[i] != prevPerformance
					.getNumberOfImprovingBestMoves()[i]) {
				tempNumOfNewBestSlnFoundInTheLastPhase += (currPerformance.getNumberOfImprovingBestMoves()[i]
						- prevPerformance.getNumberOfImprovingBestMoves()[i]);
			}
		}
		/******************************************************************************************************************/
		...
		for (int i = 0; i < numberOfHeuristics; i++) {
			...
			if (tempExecTime != 0.0 && selection.getTabuDurationList()[i] == 0) {
				...
				if (tempNumOfNewBestSlnFoundInTheLastPhase > 0) {
					performanceRate[i] = (Math.pow(
							1.0 + (currPerformance.getNumberOfImprovingBestMoves()[i]
									- prevPerformance.getNumberOfImprovingBestMoves()[i]),
							Vars.powerOfForNewBestSolutionsPerfM0) * (timeRemaining / tempExecTime) * 100000000);
				}

				performanceRate[i] += (((currPerformance.getTotalHeurImprovement()[i]
						- prevPerformance.getTotalHeurImprovement()[i]) / tempExecTime) * 100000)
						- (((currPerformance.getTotalHeurWorsening()[i] - prevPerformance.getTotalHeurWorsening()[i])
								/ tempExecTime) * 0.0001)
				...
				worseningRate[i] = (((currPerformance.getTotalHeurWorsening()[i]
						- prevPerformance.getTotalHeurWorsening()[i]) / tempExecTime));

				timeExecList[i] = tempExecTime
						/ (currPerformance.getNumberOfMoves()[i] - prevPerformance.getNumberOfMoves()[i]);
			} else {
		...
		} else if ((currPerformance.getFCNumberOfImprovingBestMoves()
				- prevPerformance.getFCNumberOfImprovingBestMoves()) == 0) {
			if (fcModeIsPrevTabu) {
		...
		selection.update(performanceRate, worseningRate, timeExecList, currPerformance, prevPerformance,
				learningRateMultiplierList); // new method @12052011
	 ...
	 * 
	 * @param performance
	 *            the performance element showing the current performance of the
	 *            heuristics in terms of improvement capabilities
	...
	private void setHeuristicClassType(PerformanceElements performance, int heuristicIndex) {
		if (performance.getTotalHeurImprovement()[heuristicIndex] > 0) {
			...
			} else if (performance.getTotalHeurImprovement()[heuristicIndex] >= performance
					.getTotalHeurWorsening()[heuristicIndex]) {
	...
	private void printCurrentState() {
		log.info("  >> iter=" + numberOfIterations + " (rst=" + numberOfRestarts + ") new=" + newFitness + " curr="
				+ currentFitness + " best=" + bestFitness + " :: (" + (System.currentTimeMillis() - startTime) / 1000F
				+ " | " + totalExecTime / 1000F + ") @ phL=" + Vars.phaseLength + " (nonTabuHr#="
				+ (numberOfHeuristics - selection.getNumberOfTabuHeuristics()) + " outOf " + numberOfHeuristics + ") "
				+ " #OfNBestAll=" + totalNumOfNewBestFound + " [selPow=" + fcSelectPow + "] " + " => FC="
				+ numberOfFCNewBest + " (NotActual=" + numberOfFCNewBestNotActual + ") TbSize=" + fcModeTabuDurationSize
				+ " [fcMod=" + fcMode + " : TD=" + fcModeTabuDuration);
	}

	/**
	 * Get current state of the search in a string (for writing to the
	...
	private String getCurrentStateForBestAsStr(int heurIndex) {
		return numberOfIterations + ";" + ("" + ((System.currentTimeMillis() - startTime) / 1000F)).replace(".", ",")
				+ ";" + ("" + bestFitness).replace(".", ",") + ";" + heurIndex + ";"
				+ (heurIndex != -1 ? (newBestByRelay ? 1 : 0) : -1) + ";" + currPerformance.getNumberOfMovesAsStr()
				+ ";" + selection.getProbabilityListAsString().replace(".", ",");
	}

	/**
	 * Get the parameter values of the heuristics in a string (for writing to
	...
	private String getCurrentStateForLOCAsStr() {
		return numberOfIterations + ";" + ("" + ((System.currentTimeMillis() - startTime) / 1000F)).replace(".", ",")
				+ ";" + ("" + bestFitness).replace(".", ",") + ";"
				+ selection.getLevelOfChangeListAsString().replace(".", ",") + ";" + getHeuristicClassTypesAsString();
	}

	/**
	 * Get the speed ratios of the heuristics in a string (for writing to the
	 * corresponding .csv file)
	 * 
	...
	/**
	 * Get the improvement range (lowest improvement-higher improvement) of the
	 * heuristics in a string (for writing to the corresponding .csv file)
	 * 
	...
	/**
	 * Get the heuristics' performance details in a string (for writing to the
	 * corresponding .csv file)
	 * 
	...
	private String getHeuristicCounterDetailsAsStr() {
		...
		str = numberOfIterations + ";"
				...
				+ ("" + bestFitness).replace(".", ",") + ";";
		for (int i = 0; i < numberOfHeuristics; i++) {
			str += currPerformance.getNumberOfMoves()[i] + ";" + currPerformance.getNumberOfImprovingBestMoves()[i]
					+ ";" + currPerformance.getNumberOfImprovingMoves()[i] + ";"
					+ currPerformance.getNumberOfEqualMoves()[i] + ";" + currPerformance.getNumberOfWorseningMoves()[i]
	...
	/**
	 * Get the current iteration limit and the threshold values of the
	 * acceptance mechanism in a string (for writing to the corresponding .csv
	 * file)
	 * 
	...
	/**
	 * Get the current status of the acceptance mechanism (for writing to the
	 * corresponding .csv file)
	 * 
	 * @return the details about the current status of the acceptance mechanism
	 */
	private String getAcceptanceDetailsAsStr() {
		return numberOfIterations + ";" + ("" + ((System.currentTimeMillis() - startTime) / 1000F)).replace(".", ",")
				+ ";" + ("" + bestFitness).replace(".", ",") + ";" + ("" + currentFitness).replace(".", ",") + ";"
				+ ("" + acceptance.getCurrentThreshold()).replace(".", ",");
	}

	/**
	 * Get the current status of the relay hybridisation (for writing to the
	 * corresponding .csv file)
	 * 
	 * @return the details about the current status of the relay hybridisation
	 */
	private String getRelayDetailsAsStr(int relayFirstHeurIndex, int relaySecondHeurIndex, double relayFirstFitness,
			double relaySecondFitness) {
		return numberOfIterations + ";"
				+ ("" + ((System.currentTimeMillis() - this.startTime) / 1000F)).replace(".", ",") + ";"
				+ ("" + newFitness).replace(".", ",") + ";" + ("" + currentFitness).replace(".", ",") + ";"
				+ ("" + relayFirstFitness).replace(".", ",") + ";" + ("" + relaySecondFitness).replace(".", ",") + ";"
				+ relayFirstHeurIndex + ";" + relaySecondHeurIndex + ";"
				+ ("" + selection.getLevelOfChangeList()[relayFirstHeurIndex]).replace(".", ",") + ";"
				+ ("" + selection.getLevelOfChangeList()[relaySecondHeurIndex]).replace(".", ",") + ";"
				+ numberOfNewBestForAllFC + ";" + numberOfFCNewBest + ";" + spentIterForRelay + ";"
				+ spentIterForValidRelay + ";" + ("" + (spentTimeForRelay / 1000000000F)).replace(".", ",") + ";"
				+ getAfterListForRelayAsString() + ";" + selection.getProbListForRelayAsString().replace(".", ",");
	}

	/**
	 * Get the current status of the relay hybridisation in case of no new best
	 * solution is found (for writing to the corresponding .csv file)
	 * 
	 * @return the details about the current status of the relay hybridisation
	 */
	private String getRelayDetailsAsStrJustInfo() {
		return numberOfIterations + ";"
				+ ("" + ((System.currentTimeMillis() - this.startTime) / 1000F)).replace(".", ",") + ";"
				+ "-1;-1;-1;-1;-1;-1;-1;-1;" + numberOfNewBestForAllFC + ";" + numberOfFCNewBest + ";"
				+ spentIterForRelay + ";" + spentIterForValidRelay + ";"
				+ ("" + (spentTimeForRelay / 1000000000F)).replace(".", ",") + ";" + getAfterListForRelayAsString()
				+ ";" + selection.getProbListForRelayAsString().replace(".", ",");
	}

	/**
	 * Get the list of second heuristic for each heuristic in a string form (for
	 * writing to the corresponding .csv file)
	 * 
	...
	/**
	 * The current types of the heuristics (for writing to the corresponding
	 * .csv file)
	 * 
