...
public class AdaptiveIterationLimitedListBasedTA extends AcceptanceCriterion {
	 ...
	 * 
	 * @param listLength
	 *            length of the threshold list
	 * @param initialFitness
	 *            fitness value of the initial solution
	 * @param adaptationLimitMultiplier
	 *            a multiplier to decide about when to increase the threshold
	 *            level
	 * @param totalExecTime
	 *            the total execution time allowed
	 */
	public AdaptiveIterationLimitedListBasedTA(int listLength, double initialFitness, int adaptationLimitMultiplier,
			long totalExecTime, Random r) {
		super(r);
		...
		iterationLimit = Vars.iterationLimitLB; /*
												 * Set initially the iteration
												 * limit to its lower bound
												 */
		movingAvgImpBestIteration = iterationLimit; /* */
		/*
		 * Set the iteration limit (K) which is used to increase the threshold
		 * level
		 */
		iterationLimitForThresholdChange = adaptationLimitMultiplier * iterationLimit;

		tempTimeFactor = 1.0; /*
								 * Initialiase the time factor (linearly decreases
								 * to zero (0))
								 */
	 ...
	 * @param bestFitness
	 *            fitness value of the current best solution
	...
	public boolean accept(double newFitness, double currentFitness, double bestFitness) {
		...
		if (newFitness < currentFitness) {
			...
			if (newFitness < bestFitness) {
				...
				/*
				 * Add the fitness value of the new best solution to the
				 * threshold list
				 */
				...
				/*
				 * Reset the the counter used to check the threshold level
				 * increase
				 */
				limitCounterForThresholdChange = 0;
				/*
				 * Set the thresholdIndex to the lowest value (preceding value
				 * to the new best solution's fitness)
				 */
				thresholdIndex = 1;
				...
				/*
				 * Calculate the number of iterations passed to find the new
				 * best solution starting when the earlier best solution was
				 * found
				 */
				...
				if (passedIter <= iterationLimit) {
					movingAvgImpBestIteration = (int) Math
							.ceil((double) (((iterAdaptationListLength - 1) * iterationLimit + passedIter)
									/ iterAdaptationListLength));
				} else {
					double tempNewBestReqIter = 0.0;

					/*
					 * if a worsening solution was not found even if passedIter
					 * >> iterationLimit
					 */
					...
					/*  */
					movingAvgImpBestIteration = (int) Math
							.ceil((double) (((iterAdaptationListLength - 1) * iterationLimit + tempNewBestReqIter)
									/ iterAdaptationListLength));

					if (Print.acceptance) {
						log.info("iterationLimit=" + iterationLimit + "  ... tempNewBestReqIter=" + tempNewBestReqIter
								+ " ... tempTimeFactor=" + tempTimeFactor + " :: consecWorseMoveAcceptanceCounter="
								+ consecWorseMoveAcceptanceCounter + " :: movingAvgImpBestIteration="
								+ movingAvgImpBestIteration + " :: passedIter=" + passedIter);
					}
				}
				...
				/*
				 * Set iteration limit (k) to its predetermined lower bound if
				 * required
				 */
				...
				if (Print.acceptance) {
					log.info("IterLimit updated = " + iterationLimit + " :: iterLimitThresholdChange updated = "
							+ iterationLimitForThresholdChange);
				}

				/*
				 * Set the current iteration number as the most recent iteration
				 * number when a new best solution was found
				 */
		...
		} else if (newFitness == currentFitness) { // If the qualities of the
													// new solution and current
													// solution are the same
			acp = true;
		} else if (limitCounter >= iterationLimit) { // If the new solution is
														// worse than the
														// current solution
			...
			if (newFitness != currentFitness && newFitness != bestFitness && newFitness <= threshold) {
				...
				if (Print.acceptance) {
					log.info("A Worsening Move Accepted: " + newFitness + " :: THRESHOLD = (" + threshold
							+ ") - iterLimit=" + iterationLimit + " (threshInx=" + thresholdIndex + " : ListLength="
							+ listLength + " )");
				}
		...
		/*
		 * Increase the threshold level if the iteration counter reaches to its
		 * limit
		 */
			...
			if (thresholdIndex <= bestFitnessList.size() - 1) {
				...
				if (Print.acceptance) {
					log.info(" TO " + threshold);
						 ...
						 * If the threshold level is reached to its maximum level
						 */
				...
				if (Print.acceptance) {
					log.info(
							"Search is at stuck (explore better level of change values for the heuristics' parameters) !");
				}
				...
				if (!Vars.useOverallBestSln && !Vars.noMoreRestart) {
					...
					} else if (!Vars.noMoreRestart) { // RESTART is impossible
														// during further
														// iterations
	...
	public void resetAcceptanceList(double newFitness) {
		...
		/*
		 * Set the threshold index to the lowest threshold value's index (0: is
		 * the index of the new best solution)
		 */
		thresholdIndex = 1;
		/* Set the threshold level */
	 ...
	 * 
	 * @param bestFitnessList
	 *            the list of recently found best solutions (threshold list)
	 */
	private void updateAcceptanceListForNoMoreRestartCase(Vector<Double> bestFitnessList) {
		if (bestFitnessListForNoMoreRestartCase.size() == 0) {
			...
			// If the new found best solution is better than the one came from
	...
	private void updateListLength() {
		double timeFactor = (double) (Vars.totalExecutionTime - (System.currentTimeMillis() - startTime))
				/ Vars.totalExecutionTime;

		listLength = (int) (Vars.aillaListSize / 2)
				+ (int) (((int) (Vars.aillaListSize / 2) + 1) * (timeFactor * timeFactor * timeFactor));

		int tempBestListSize = bestFitnessList.size();
		if (listLength < tempBestListSize) {
			for (int i = tempBestListSize; i > listLength; i--) {
				if (bestFitnessList.size() > 0) {
					bestFitnessList.remove(bestFitnessList.size() - 1);
				}
			}
		}
